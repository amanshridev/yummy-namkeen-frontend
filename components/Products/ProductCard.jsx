    ...
    return (
        <div key={item.id} className="card w-72 bg-base-100 border border-[#FE9400] rounded-md mt-8 bg-inherit">
            <figure>
                {item.imageUrl && (
                    <img
                        src={item.id ? item.imageUrl : "https://dummyimage.com/600x400/000/fff" }
                        alt={item.itemName}
                        className="w-full h-44 p-2 object-cover rounded-xl"
                        width={40}
                        height={40}
                    />
                )}
            ...
            <div className="card-body p-3">
                ...
                <div className="card-actions justify-center mt-2">
                    <button className="btn bg-red-500 border-none hover:bg-orange-500 w-44 text-white"> <Icon icon="material-symbols:shopping-cart" color="white" /> &nbsp; Add to cart</button>
                </div>
            </div>
        </div>
    )
